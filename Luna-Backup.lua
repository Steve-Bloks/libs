...
function Luna:Notification(data)
    task.spawn(function()
        data = Kwargify({
            ...
            Duration = nil -- Optional: Specify custom duration
        }, data or {})
        ...
        -- Calculate text bounds and set initial values
        ...
        -- Calculate duration or fallback to default
        local waitDuration = data.Duration or math.min(math.max((#newNotification.Description.Text * 0.1) + 2.5, 3), 10)
...
function Luna:CreateWindow(WindowSettings)
	...
	local Window = { Bind = Enum.KeyCode.RightShift, CurrentTab = nil, State = true, Size = false, Settings = nil }
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Luna Interface Suite" and Release or ""
...
if isStudio then
	local Window = Luna:CreateWindow({
		...
		Subtitle = "by a literal baseplate.",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "a literal baseplate.",
		LoadingSubtitle = "Loading script for a literal baseplate.",
		KeySystem = true,
		KeySettings = {
			Title = "a literal baseplate. | Key System",
			Subtitle = "a literal baseplate.",
			Note = "Please Enter Your Key To Use This Script",
			FileName = "Key",
			SaveKey = true,
			Key = {"Example Key"},
			SecondAction = {
